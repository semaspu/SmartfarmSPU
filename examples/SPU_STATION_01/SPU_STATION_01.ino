...
//=================================================================================================
#include "src/ET_HT16K33_7SEGMENT.h"
...
//=================================================================================================
#include "src/ModbusMaster.h"             // https://github.com/4-20ma/ModbusMaster
//=================================================================================================
...
void setup_wifi() {
  ...
  SerialDebug.println(SECRET_SSID);
  WiFi.mode(WIFI_STA);
  WiFi.begin(SECRET_SSID, SECRET_OPTIONAL_PASS);
  while (WiFi.status() != WL_CONNECTED) {
    ToggleLedStatus(wifi);
    delay(500);
    SerialDebug.print(".");
    ToggleLedStatus(wifi);
    delay(500);
    countReboot ++;
    if(countReboot>rebootTime) ESP.restart();
...
void setup() {
    ...
    setup_wifi();
      ...
      //SerialDebug.println("go to here ok");    while(1);
...
void loop() {
          ...
          // Read Opto 
          if(ReadOptoInput(0)== HIGH)  OnLedSw(0)   ; else OffLedSw(0)  ;
          if(ReadOptoInput(1)== HIGH)  OnLedSw(0)   ; else OffLedSw(1)  ;
          if(ReadOptoInput(2)== HIGH)  OnLedSw(0)   ; else OffLedSw(2)  ;
          if(ReadOptoInput(3)== HIGH)  OnLedSw(0)   ; else OffLedSw(3)  ;         
          
          // Read Switch
          if(ReadSwitch(0)   == HIGH){  ToggleRelay(0); SerialDebug.println("Key1 Press");  while(ReadSwitch(0)== HIGH);  }
          if(ReadSwitch(1)   == HIGH){  ToggleRelay(1); SerialDebug.println("Key2 Press");  while(ReadSwitch(1)== HIGH);  }
          if(ReadSwitch(2)   == HIGH){  ToggleRelay(2); SerialDebug.println("Key3 Press");  while(ReadSwitch(2)== HIGH);  }
          if(ReadSwitch(3)   == HIGH){  ToggleRelay(3); SerialDebug.println("Key4 Press");  while(ReadSwitch(3)== HIGH);  }
   


          if (ArduinoCloud.connected() == 0) {
              SerialDebug.println("not connected do nothing or whatever");
              ToggleLedStatus(link);
              delay(500);
              ToggleLedStatus(link);
              delay(500);
              countReboot ++;
    ...
    else if(indexGetModbus == 2)
    {
      ...
      if(read_modbus_status == nodeSoilMoisture.ku8MBSuccess)
      {
        ...
        soilTDS = (uint16_t)nodeSoilMoisture.getResponseBuffer(8);
          ...
          soil_mois=soilMoisture;
          soil_ec=soilEc;
          soil_nitro=soilNitrogen;
          soil_ph=soilPh;
          soil_phos=soilPhosphorus;
          soil_potas=soilPotassium;
          soil_tds=soilTDS;
          soil_salin=soilSalinity;
...
/*
  Since Sw0 is READ_WRITE variable, onSw0Change() is
